...
// tag num           yzhu:这边的tag num 表示T的数量，如果有bichar trichar，那么tag num 为2 
...
// tag Alphabet fixed  
 ...
 // cout << "##################################################################end of createAlphabet##############################" << endl ;
...
void Labeler::extractFeature(Feature& feat, const Instance* pInstance, int idx) {
 ...
 // cout << "TTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTTT" << endl;
 // cout << "tagNum::" << tagNum << endl;
...
void Labeler::convert2Example(const Instance* pInstance, Example& exam) {
  ...
  for (int i = 0; i < curInstSize; ++i) {
    ...
    //cout << exam.m_features.size() << endl;
    //cout << "11111111111111111111111111" << endl;
...
void Labeler::train(const string& trainFile, const string& devFile, const string& testFile, 
    const string& modelFile, const string& optionFile, const string& wordEmbFile , const string& tagEmbFiles) {
  if (optionFile != "")
  ...
  createAlphabet(trainInsts);//统计词频以及一些特征的频率，并创建对应的字典
...
void Labeler::readTagEmbeddings(const string& inFile,NRMat<dtype>& tagEmb,  Alphabet& p_tagAlphabet)
{
	static ifstream inf;
	if(inf.is_open())
	{
	...
	int unknownId = p_tagAlphabet.from_string(unknownkey);//这边需要看一下tagAlphabet里面有没有这个key 
	static vector<string> vecInfo;
	split_bychar(strLine, vecInfo, ' ');
	int tagDim = vecInfo.size() - 1;
	std::cout << "tag embedding dim is " << tagDim << std::endl;
	m_options.tagEmbSize = tagDim;

	tagEmb.resize(p_tagAlphabet.size(), tagDim);
	tagEmb = 0.0;
	curTag = vecInfo[0];
	tagId = p_tagAlphabet.from_string(curTag);
	...
	if(tagId >= 0)
	{
	  ...
	  if(unknownId == tagId)
	      bHasUnknown = true;
	  indexers.insert(tagId);
	  for (int idx =0;idx < tagDim;idx++)
	  {
	    dtype curValue = atof(vecInfo[idx + 1].c_str());
	    sum[idx] = curValue;
            tagEmb[tagId][idx] = curValue;
	  }
	...
	while (1)
	{
	  ...
	  if(strLine.empty())
	    continue;
	  split_bychar(strLine, vecInfo , ' ');
	  if(vecInfo.size() != tagDim + 1)
	  {
	    std::cout << "error tag embedding file" << std::endl;
	  }

	  curTag = vecInfo[0];//normalize_to_lowerwithdigit(vecInfo[0])
	  tagId = p_tagAlphabet.from_string(curTag);
	  if(tagId >= 0)
	  {
	    count++;
	...
	if (!bHasUnknown) 
	{
	  for (int idx = 0; idx < tagDim; idx++) 
	  {
	    tagEmb[unknownId][idx] = sum[idx] / count;
          }
    	  count++;
  	...
  	int oovTags = 0;
  	int totalTags = 0;
  	for (int id = 0; id < p_tagAlphabet.size(); id++) 
	{
    	  if (indexers.find(id) == indexers.end()) 
	  {
       	    oovTags++;
	    for (int idx = 0; idx < tagDim; idx++){
	    tagEmb[id][idx] = tagEmb[unknownId][idx];
						            }
	  }
    	  totalTags++;
    	}

   	std::cout << "OOV num is " << oovTags << ", total num is " << p_tagAlphabet.size() << ", embedding oov ratio is " << oovTags * 1.0 / p_tagAlphabet.size() << std::endl;
/*
*/	
  }
...
int main(int argc, char* argv[]) {
	...
	std::string wordEmbFile = "", optionFile = "" , tagEmbFiles = "";
	...
	ah.new_named_string("tag", "tagEmbFiles", "named_string",
			"tag file to train a model, optional when training", tagEmbFiles);
	...
	Labeler tagger;
	if (bTrain) {
		tagger.train(trainFile, devFile, testFile, modelFile, optionFile, wordEmbFile, tagEmbFiles);
	} else {
