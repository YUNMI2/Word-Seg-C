...
workspace=`pwd`
tooldir=$workspace/../../NNSegmentation-zhang-ctb-baseline-0.001
corpus=ctb
...
function runLSTM
{
    ...
    mkdir $workspace/$outputdir/$cmd -p
    ln -s $workspace/$corpus/$corpus.train.bichar.feats $workspace/$outputdir/$cmd/$corpus.train.feats 
    ln -s $workspace/$corpus/$corpus.dev.bichar.feats $workspace/$outputdir/$cmd/$corpus.dev.feats 
    ln -s $workspace/$corpus/$corpus.test.bichar.feats $workspace/$outputdir/$cmd/$corpus.test.feats 
    cp $tooldir/$cmd $workspace/$outputdir/$cmd/
    train_file=$workspace/$outputdir/$cmd/$corpus.train.feats
    dev_file=$workspace/$outputdir/$cmd/$corpus.dev.feats
    test_file=$workspace/$outputdir/$cmd/$corpus.test.feats
