...
void Labeler::train(const string& trainFile, const string& othertrainFile, const string& devFile, const string& testFile, const string& modelFile, const string& optionFile) {
  	if (optionFile != "")
    		m_options.load(optionFile);
  	m_options.showOptions();//配置文件

  
  	vector<Instance> allSampleTrainInstances, trainInsts, devInsts, testInsts, othertrainInsts, alltrainInsts;
	...
	vector<size_t> othertrainInsts_posi;
	if (othertrainFile != "")
	...
	createAlphabet(trainInsts, othertrainInsts_posi, othertrainFile);
  	...
  	vector<Example> trainExamples, othertrainExamples, devExamples, testExamples, allSampleTrainExamples;
	...
	cout << "trainSampleSize: " << trainSampleSize << "\t\t" << "othertrainSampleSize: " << othertrainSampleSize << endl;
	...
	static vector<Instance> allSampleInstances;
...
int main(int argc, char* argv[]) {
	...
	std::string trainFile = "", othertrainFile = "", devFile = "", testFile = "", modelFile = "";
	...
	Labeler tagger;
	if (bTrain) {
		tagger.train(trainFile, othertrainFile, devFile, testFile, modelFile, optionFile);
	} else {
